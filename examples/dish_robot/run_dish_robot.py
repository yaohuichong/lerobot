    ...
    def start_episode(self, dish_name: str = "unknown", filename: str = None):
        ...
        if filename:
            ...
            filepath = os.path.join(
                self.log_dir, f"{timestamp}_{dish_name}_episode{self.current_episode}.txt"
            )

        self.log_file = open(filepath, "w")
        self.log_file.write(
            "# timestamp,step,shoulder_pan,shoulder_lift,elbow_flex,wrist_flex,wrist_roll,gripper,action_pan,action_lift,action_elbow,action_flex,action_roll,action_gripper\n"
        )
        self.step_count = 0
...
class RealRobotController:
    def __init__(
        self, port: str = None, fps: int = 30, smoothing_alpha: float = None, log_joints: bool = True
    ):
