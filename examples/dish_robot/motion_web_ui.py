    ...
    def get_status(self):
        ...
        if self.is_executing:
            if self._total_count > 0:
                progress = "执行中 [{}/{}]: {}".format(
                    self._current_index, self._total_count, self.current_dish
                ) if self.current_dish else "执行中 [{}/{}]".format(self._current_index, self._total_count)
                return progress
...
class MotionWebUI:
    ...
    def _play_dish(self, dish_name):
        ...
        return "[OK] 开始执行: {}".format(dish_name), self.controller.get_status(), self.controller.get_queue_status(), 0
    ...
    def _add_to_queue(self, dish_name, count):
        ...
        return "[OK] 已添加 {} 份 {}，队列共 {} 项".format(count, dish_name, queue_len), self.controller.get_queue_status(), 0
    ...
    def create_ui(self):
        ...
        with gr.Blocks(title="打饭机器人 - 动作控制", theme=gr.themes.Soft(), css=css) as demo:
            ...
            def start_with_timer():
                result = self.controller.start_queue_execution(fps=30)
                return result, self.controller.get_status(), self.controller.get_queue_status(), 0, gr.Timer(active=True)
            ...
            def stop_with_timer():
                result = self.controller.stop()
                return result, self.controller.get_status(), self.controller.get_queue_status(), 0, gr.Timer(active=False)
